...
class Transaction:
    ...
    def recalculate_window(self, timestamp):
        ...
        log.debug("Latency: %.8f\tGain: %.2f\tWindow: %i\tSent: %i\tAcknowledged: %i\tIn Flight: %i\tDelay: %.4fs\tTransfered: %.0f%%" % st(self.latency, gain, self.window_size, self.sent_chunks, self.acknowledgements, len(self.chunks_in_flight), (time.time() - timestamp), (self.acknowledgements/float(self.size))*100))
